...
const AdminCompanyProfileLayout: React.FC = ({ id }: any) => {
    ...
    const [validationLogoMessage, setValidationLogoMessage] = useState<{ [key: string]: string}>({})
        ...
        <>
            ...
            <form onSubmit={validateEditLogo} encType='multipart/form-data'>
                <div className='grid grid-cols-1 gap-4'>
                    <div className='flex flex-col'>
                        <label htmlFor='logo' className='text-sm font-semibold text-gray-500'>Logo KBIHU</label>
                        ...
                        { validationLogoMessage.logo && <p className='text-sm text-red-500'>{validationLogoMessage.logo}</p> }
                        { companyLogo && <img src={`/upload_files/profile_pic/${companyLogo}`} alt='logo' className='w-16 object-contain my-2' /> }
                    </div>
                </div>
                <button className="mt-2 bg-blue-500 text-white active:bg-blue-600 font-bold uppercase text-xs px-4 py-2 rounded shadow outline-none focus:outline-none">
                    <FontAwesomeIcon icon={faPen} />
