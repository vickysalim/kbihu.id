...
const validation = z.object({
  ...
  departure_group: z.coerce.number().optional(),
  embarkation: z.string().optional(),
});
    ...
    if (!userExist && companyValid) {
      ...
      const user_profile = await prisma.user_profile.create({
        data: {
          ...
          departure_group: reqBody.departure_group,
          embarkation: reqBody.embarkation,
        },
      });

      return api.res(res, 200, true, `Successfully add new jemaah`, {
        user_account,
        user_profile,
      });
    }
